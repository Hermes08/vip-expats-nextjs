...
export default function Home() {
  ...
  return (
    <>
      ...
      <section className="py-20 bg-gradient-to-b from-black to-brand-950 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            {/* Image Side */}
            <div className="relative group">
              <div className="absolute -inset-4 bg-gradient-to-r from-brand-GOLD/20 to-brand-GOLD/5 rounded-[3rem] blur-2xl group-hover:blur-3xl transition-all duration-500"></div>
              <div className="relative bg-gradient-to-br from-white/5 to-white/[0.02] backdrop-blur-xl rounded-[3rem] p-8 border border-white/10 overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-brand-GOLD/10 rounded-full blur-3xl"></div>
                <img
                  src="/images/team-member.png"
                  alt="Senior Real Estate Advisor - Panama Investment Expert"
                  className="w-full h-auto relative z-10 drop-shadow-2xl"
                />
                <div className="absolute bottom-8 left-8 right-8 bg-brand-950/90 backdrop-blur-md p-6 rounded-2xl border border-brand-GOLD/20">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-brand-GOLD rounded-full flex items-center justify-center">
                      <CheckCircle className="text-brand-950" size={24} />
                    </div>
                    <div>
                      <p className="text-white font-black text-sm uppercase tracking-wider">Verified Expert</p>
                      <p className="text-brand-GOLD text-xs font-bold">15+ Years Experience</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="text-white">
              <span className="text-brand-GOLD font-black uppercase tracking-[0.4em] text-[10px] mb-4 block">Your Trusted Partner</span>
              <h2 className="font-heading text-4xl md:text-6xl font-black mb-8 uppercase tracking-tighter italic leading-tight">
                WORK WITH <span className="text-brand-GOLD">PANAMA'S TOP</span> ADVISORS
              </h2>
              <p className="text-slate-300 text-lg mb-8 leading-relaxed font-medium">
                Our senior advisors have helped over <strong className="text-white">500+ international investors</strong> secure their dream properties in Panama. From residency visas to property management, we handle everything.
              </p>

              <div className="space-y-6 mb-10">
                {[
                  { icon: ShieldCheck, title: "Licensed & Insured", desc: "Fully certified real estate professionals" },
                  { icon: Globe, title: "Multilingual Team", desc: "English, Spanish, and Mandarin support" },
                  { icon: TrendingUp, title: "Proven Track Record", desc: "$50M+ in successful transactions" }
                ].map((item, i) => (
                  <div key={i} className="flex items-start gap-4 group/item">
                    <div className="w-12 h-12 bg-brand-GOLD/10 rounded-xl flex items-center justify-center shrink-0 group-hover/item:bg-brand-GOLD/20 transition-colors">
              ...
              <Link
                href={`/${lang}/contacto`}
                className="inline-flex items-center gap-3 bg-brand-GOLD text-brand-950 px-10 py-5 rounded-full font-black uppercase tracking-widest text-xs hover:bg-white transition-all shadow-2xl shadow-brand-GOLD/20"
              >
