...
import { Map, Waves, TrendingUp, Palmtree, ArrowRight, ShieldCheck } from 'lucide-react';
...
interface BocasPageContentProps {
  lang: string;
}

const BocasPageContent = ({ lang }: BocasPageContentProps) => {
  const isEn = lang === 'en';
  ...
  const bocasProjects = PROJECTS.filter(p =>
    p.location?.en?.toLowerCase().includes('bocas') ||
    p.location?.es?.toLowerCase().includes('bocas')
  ).slice(0, 3);
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
          ...
          <div className="grid lg:grid-cols-12 gap-32 items-center">
            <div className="lg:col-span-12 reveal-on-scroll mb-20">
              <div className="text-center max-w-4xl mx-auto">
                <h2 className="text-5xl md:text-8xl font-heading font-black text-brand-950 mb-10 tracking-tighter uppercase italic leading-[0.8]">
                  {isEn ? "The Panama Archipelago Advantage" : "La Ventaja del Archipiélago de Panamá"}
                </h2>
              ...
              {content.features.map((feature, i) => (
                <div key={i} className="flex gap-12 items-start reveal-on-scroll stagger-1 group">
                  ...
                  <div>
                    ...
                    <p className="text-slate-500 font-medium leading-relaxed italic opacity-80">{feature.desc}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <section className="py-48 bg-brand-50 rounded-[6rem] border border-brand-100 relative overflow-hidden text-center px-4">
          <div className="reveal-on-scroll">
            ...
            <h2 className="text-5xl md:text-9xl font-heading font-black text-brand-950 mb-16 tracking-tighter uppercase italic leading-[0.8]">
              {isEn ? "Luxury Island Assets" : "Activos de Lujo en la Isla"}
            </h2>
          </div>
...
export default BocasPageContent;
